...
class CartActivity : ComponentActivity() , View.OnClickListener, CartItemClickListener {
    ...
    override fun onClick(p0: View?) {
        ...
        if(!(savedUsername.isNullOrEmpty())) {
            ...
            if(loggedInUser.address.isNullOrEmpty()){
                Toast.makeText(
                    this,
                    "Cím megadása nélkül nem tudsz rendelni!",
                    Toast.LENGTH_SHORT
                ).show()
            }else{
    ...
    private fun getDataFromDb(){
        CoroutineScope(Dispatchers.IO).launch {
            this.let {
                var cat = BookDB.getDatabase(this@CartActivity).getCartDao().getAllCartItems()
    ...
    private fun updateTotalSumAndQuantity() {
        runOnUiThread {
            ...
            sumQuantity.text = "Összes darabszám: " + totalQuantity.toString()
    ...
    override fun onItemDelete(cart: Cart){
        ...
        totalSum -= cart.quantity * cart.bookAdded.price
    ...
    private fun setBottomSum(){
            CoroutineScope(Dispatchers.IO).launch {
                this.let {
                    var cat =
                        BookDB.getDatabase(this@CartActivity).getCartDao().getAllCartItems()
                    arrCarts = cat as ArrayList<Cart>
                    ...
                    if (arrCarts.size <= 0) {
                        ...
                        for (cart: Cart in arrCarts) {
                            ...
                            if (cart.quantity == 1) {
                                sum += cart.bookAdded.price
                                quantity++
                            } else if (cart.quantity > 1) {
                                sum += cart.bookAdded.price * cart.quantity
