...
class RegisterActivity : AppCompatActivity() {
    ...
    private lateinit var myDb : BookDB
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        myDb = Room.databaseBuilder(this, BookDB::class.java, "myApp2")
    ...
    private fun validateEmptyForm(){
        ...
        when {
                TextUtils.isEmpty(fullName.text.toString().trim()) -> {
                    fullName.error = "Kérjek, add meg a neved!"
                }

                TextUtils.isEmpty(username.text.toString().trim()) -> {
                    username.error = "Kérlek, adj meg egy felhasználónevet!"
                }

                TextUtils.isEmpty(phoneNumber.text.toString().trim()) -> {
                    phoneNumber.error = "Kérlek, ajd meg egy telefonszámot!"
                }

                TextUtils.isEmpty(password.text.toString().trim()) -> {
                    password.error = "Kérlek, adj meg egy jelszót!"
                }

                TextUtils.isEmpty(passwordAgain.text.toString().trim()) -> {
                    passwordAgain.error = "Kérlek, add meg a jelszót újra!"
                }
                            ...
                            if(phoneNumber.text.toString().matches(Regex("^[\\+]?[(]?[0-9]{3}[)]?[-\\s\\.]?[0-9]{3}[-\\s\\.]?[0-9]{4,6}\$"))) {
                                if (password.text.toString().length >= 6) {
                                    if (password.text.toString() == passwordAgain.text.toString()) {
                                        ...
                                        passwordAgain.error = "A jelszavak nem egyeznek!"
                                    }
                                } else {
                                    password.error = "A jelszónak legalább 6 karakter hosszúnak kell lennie!"
                                }
                            }else{
                                phoneNumber.error = "Hibás telefonszám formátum!"
                            }
                        } else {
                            username.error = "Ezzel az email-címmel már korábban regisztáltak egy felhasználót!"
                        }
                    } else {
                        username.error = "Hibás email-cím formátum!"
                    }
